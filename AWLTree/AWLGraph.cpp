#include "AWLGraph.hpp"

AWLGraph::AWLGraph(sf::RenderWindow &window) : window(window),
...
bool AWLGraph::eventTest()
{
    ...
    while (window.pollEvent(event))
    {
        ...
        if (event.type == sf::Event::MouseButtonPressed && event.mouseButton.button == sf::Mouse::Left)
        {
            ...
            {
                timer.startTimer();
                tree.updateNode(std::stoi(updateInputId.getValue()), std::stoll(updateInputTime.getValue()), std::stof(updateInputPrice.getValue()), updateInputName.getValue());
                timer.endTimer();
                timerOutput();
                updatePages();
            }
            ...
            {
                timer.startTimer();
                AWLNode *found = tree.getNode(std::stoll(searchInput.getValue()));
                timer.endTimer();
                timerOutput();
                page[0] = found;
                page[1] = nullptr;
...
void AWLGraph::timerOutput()
{
    timeOutputText.setString("  " + std::to_string(timer.getTime()) + "uS");
}

void AWLGraph::updatePages()
{
    page = tree.getPageElements(pageNumber, page[0]);
}

void AWLGraph::treeOutput()
{
    ...
    if (page[0] != nullptr)
    {
        treeNodeText1.setString(
            ...
            "\n\nBalance index " + std::to_string(page[0]->getIndex()));
    }
    else
    {
    ...
    if (page[1] != nullptr)
    {
        treeNodeText2.setString(
            ...
            "\n\nBalance index " + std::to_string(page[1]->getIndex()));
    }
    else
    {
    ...
    if (page[2] != nullptr)
    {
        treeNodeText3.setString(
            ...
            "\n\nBalance index " + std::to_string(page[2]->getIndex()));
    }
    else
    {
